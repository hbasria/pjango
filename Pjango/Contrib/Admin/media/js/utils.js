...
function fillSelect(element, options){
	...
	$.getJSON(settings.url,{responsetype: 'json'}, function(resp){
		
		var options = '<option value="">'+settings.title+'</option>';
		for (var i = 0; i < resp.results.length; i++) {
			options += '<option value="' + resp.results[i].id + '" ';
	        if(resp.results[i].id == settings.selected){
	        	options += ' selected="selected"';
	        }
	        options += '>' + resp.results[i].name + '</option>';              
		}
