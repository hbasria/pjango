...
function recursiveFolders($directory) {
    $uploadsPath = SITE_PATH.'/media/uploads/';
    $output = '';
...
class FilemanagerViews {
	...
	function image($request) {
	    ...
	    if (isset($request->POST['image'])) {
	        echo $siteUrl."/thumb/media/uploads/".$request->POST['image'];
	    }
	}	
	...
	function files($request) {
	    ...
	    $mediaRoot = SITE_PATH.'/media/uploads/';
	    $directory = $mediaRoot;
	    $siteUrl = pjango_ini_get('SITE_URL');
	    
	    if (isset($request->POST['directory']) && strlen($request->POST['directory'])>0) {
	        $directory = $mediaRoot . str_replace('../', '', $request->POST['directory']);
	    } else {
	    	if(isset($_SESSION['FileManager']['last_directory'])){
	    		//$directory = $_SESSION['FileManager']['last_directory'];
	    	}	        
	    }
	    
	    $allowedExtensions = pjango_ini_get('FILE_ALLOWED_EXTENSIONS');
	    ...
	    if ($files) {
	        foreach ($files as $file) {
	            ...
	            if (in_array(strtolower($ext), $allowedExtensions)) {
	                ...
	                $fileName = substr($file, strlen($mediaRoot));
	
	                $json[] = array(
							'file'     => substr($file, strlen($mediaRoot)),
							'filename' => basename($file),
							'size'     => round(substr($size, 0, strpos($size, '.') + 4), 2) . $suffix[$i],
							'thumb'    => $siteUrl."/thumb/media/uploads/".$fileName
	                );
	            }
	        }
	    }
	    $_SESSION['FileManager']['last_directory'] = $directory;
	    echo json_encode($json);	    
	}	
	...
	public function upload($request) {
	    ...
	    if (isset($request->POST['directory'])) {
	        if (isset($_FILES['image']) && $_FILES['image']['tmp_name']) {
	            ...
	            $maxUploadSize = pjango_ini_get('FILE_UPLOAD_MAX_MEMORY_SIZE');
	            ...
	            $allowedMimeTypes = pjango_ini_get('FILE_ALLOWED_MIME_TYPES');
	
	            if (!in_array($_FILES['image']['type'], $allowedMimeTypes)) {
	                $json['error'] = pjango_gettext('Warning: Incorrect file type!').$_FILES['image']['type'];
	            }
	            
	            $allowedExtensions = pjango_ini_get('FILE_ALLOWED_EXTENSIONS');
	
	            if (!in_array(strtolower(strrchr($_FILES['image']['name'], '.')), $allowedExtensions)) {
	                $json['error'] = pjango_gettext('Warning: Incorrect file extensions!');
	            }
