...
use Pjango\Phorm\Fields\PhormField;
...
abstract class Phorm
{
     ...
     * @author Jeff Ober
     **/
    public function __construct($data=array(), $method=Phorm::POST, $multi_part=false)
    {
        ...
        $formNameArr = explode('\\', get_class($this));
        $this->_name = $formNameArr[count($formNameArr)-1]; 
        $this->_id = 'id_'.$this->_name;
    ...
    public function as_list()
    {
        ...
        foreach ($this->fields as $name => $field)
        {
            $label = $field->label();
            if ($label !== '')
                $elts[] = sprintf('<li>%s %s %s %s</li>', $field->label(), $field->html(), $field->help_text(), $field->errors());
            else
                $elts[] = strval($field);
        }
        return implode($elts);
