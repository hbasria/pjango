 ...
 * 
 * Forms is a form library that provides a number of factory classes that
 * generate HTML form data. Forms are defined by extending the base abstract
 * Form class with a 'define_fields' method which, when called, defines the
 * form's fields as class attributes. See the examples directory for a sample
 * comment form. Forms is loosely modeled on the Django forms library, to the
 * extent that PHP is able to do the kind of introspection that it does. If you
 * are familiar with Django forms, then the concepts in Forms should not be too
 * alien.
 *
 * @author Jeff Ober
 * @package default
 ...
 **/
require_once('pjango/forms/widgets.php');
/**
 ...
 * form validation.
 **/
require_once('pjango/forms/fields.php');
 ...
 * Form
 * 
 * The abstract Form class wraps all of the functionality of the form itself.
 ...
 * Fields must be descendents of the FormField class.
 *
 * @author Jeff Ober
 * @see fields.php
 * @example ../examples/comment_form.php A simple comment form
...
abstract class Form
{
     ...
     * @author Jeff Ober
     **/
    public function __construct($data=array(), $method=Form::POST, $multi_part=false)
    {
        ...
        if (is_array($data) && is_array($user_data)){
