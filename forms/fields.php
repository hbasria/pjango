...
abstract class PhormField
{
    ...
    {
        return sprintf('<p class="phorm_help">%s</p>', pjango_htmlentities($this->help_text));
    }
...
class MultipleChoiceField extends PhormField
{
     ...
     * @param array $attributes a list of key/value pairs representing HTML attributes
     **/
    public function __construct($label, array $choices, array $validators=array(), array $attributes=array(), $widget = FALSE)
    {
        parent::__construct($label, $validators, $attributes);        
        $this->choices = $choices;
        if ($widget){
        	$this->widget = new $widget($this->choices);	
        }
