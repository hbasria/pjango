...
function pjango_htmlentities($str) {
...
class PhormWidget
{
    ...
    {
        return pjango_htmlentities((string)$str);
    }
    ...
    {
        $value = pjango_htmlentities( (string)$value );
        foreach ($attributes as $key => $val)
            $attributes[pjango_htmlentities( (string)$key )] = pjango_htmlentities( (string)$val );
...
class OptionGroupWidget extends PhormWidget
{
    ...
    protected function serialize($value, array $attributes=array())
    {
        ...
        foreach ($this->options as $actual => $display)
        {
            ...
            $html .= sprintf('%s %s', pjango_htmlentities($display), $option->html($actual, $attributes));
        }
 ...
 * @package Widgets
 **/
class MultiSelectCheckboxWidget extends PhormWidget
{
    /**
     * The choices for this field as an array of actual=>display values.
     **/
    ...
    protected function serialize($value, array $attributes=array())
    {
        ...
        foreach($this->choices as $actual => $display)
        {
            ...
            $options[] = sprintf('<label><input type="checkbox" %s value=%s>%s</label>',
            	$this->serialize_attributes($attributes),
                $this->serialize_attributes($option_attributes),
                $this->clean_string($display));
        }
