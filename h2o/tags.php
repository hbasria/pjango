...
class Repeat_Tag extends H2o_Node {
    function __construct($argstring, $parser, $pos=0) {
    	$this->args = H2o_Parser::parseArguments($argstring);
    ...
    function render($contxt, $stream) {
    	...
    	$variable = $contxt->resolve($this->args[0]);
    	$str = str_repeat($this->args[1], $variable);
    	
    	$stream->write($str);
    }
}

H2o::addTag(array('block', 'extends', 'include', 'if', 'ifchanged', 'for', 'with', 'cycle', 'load', 'debug', 'now', 'autoescape', 'csrf_token', 'repeat'));
?>